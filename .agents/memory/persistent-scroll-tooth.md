...
The composited fixed unit can run on ALL devices — no separate static mobile copy needed. Use a `gsap.matchMedia({ isDesktop: '(min-width:768px)', isMobile: '(max-width:767px)' })` context and read `ctx.conditions` to tune per-breakpoint: mobile uses a smaller horizontal sway coefficient (`kx`), a smaller base scale, and lower travel opacity so the fixed tooth never obscures section copy. Keep the fixed layer `pointer-events-none` and raise the hero card/stats to `z-40` (headline behind at `z-0`) so text stays readable/clickable behind the traveling object. Bias the container upward on mobile (`items-start` + top padding) so the tooth sits above the fold's text.
