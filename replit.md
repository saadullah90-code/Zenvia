...
- Hero tooth + splash are one perfectly-aligned unit: a fixed, viewport-centered container (shown on ALL devices) holds the splash behind and the tooth in front. On load the splash POPS up from behind (Framer spring) and the tooth DROPS in from the top (Framer, gated on `!loading`), settling with a slight rest tilt. On scroll the splash fades out (it belongs to the hero) while the tooth travels DOWN the viewport (monotonically increasing `y`) with horizontal sway and rotates (0→270deg), driven by a scrubbed GSAP timeline. Same animation on mobile via a `gsap.matchMedia` `{isDesktop,isMobile}` branch that tunes `kx`/base scale/travel opacity; the fixed tooth layer is `pointer-events-none overflow-hidden` and sits at `zIndex:30` — the TOP visual layer ABOVE the `zIndex:20` hero section (below only the `z-50` navbar) so it stays clearly visible over every (opaque) content section as it travels; because it is `pointer-events-none`, being on top never blocks clicks (see the z-layering note below). See `.agents/memory/persistent-scroll-tooth.md`.
...
- Hero z-layering is THREE sibling layers under the page root (NOT nested), ordered `backdrop z=1 < hero <section> z=20 < fixed tooth z=30 < navbar z=50`: a `zIndex:1` backdrop (gradient + glow blob + giant "Smile Matters" serif), the transparent `zIndex:20` hero `<section>` (info card + stats), and the `zIndex:30` fixed tooth ON TOP. The tooth must be the top visual layer (not behind the card/content) because every content section is opaque — a tooth placed behind them would be fully hidden, breaking the "tooth stays visible throughout scroll" requirement; `pointer-events-none` keeps clicks working underneath. Backgrounds that must sit BEHIND the tooth go in the `z=1` backdrop (a node nested in the `z=20` section forms its own stacking context and can't be pushed below the sibling fixed tooth). See `.agents/memory/hero-stacking-and-mobile.md`.
- Mobile right-shift guard: the `position:fixed` tooth escapes body overflow clipping, so its GSAP x-transform can create horizontal scroll that shifts the page right on mobile. Prevented by `overflow-hidden` on the fixed tooth container + a global `html, body, #root { max-width:100%; overflow-x:hidden }` guard in `index.css` + a reduced mobile sway coefficient (`kx`).
